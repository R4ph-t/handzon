// Load the scaffold's .env BEFORE any module that reads process.env.
// Same file the Astro site uses — single source of truth for local dev.
// dotenv silently no-ops if the file doesn't exist (production on
// Render: env comes from the platform) and never overrides values
// already set in process.env. We resolve against cwd so this works
// whether the binary is invoked as `pnpm exec handzon-ai` (cwd = scaffold
